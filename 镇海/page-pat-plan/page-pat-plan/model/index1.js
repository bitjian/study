...
// 前端操作类型
const interfaceData = [
  { key: 'addNorMalPlan', name: '常规计划', operatorName: 'addNorMalPlan', operatorCallbackName: 'callbackaddNorMalPlan', serverMethocd: addPlan, normalParam: normapParam },
  { key: 'addTempPlan', name: '临时计划', operatorName: 'addTempPlan', operatorCallbackName: 'callbackaddTempPlan', serverMethocd: addPlan },
  ...
  effects: {
    ...
    * initTable({ payload, callback }, { call, put, select, take }) {
      ...
      if (metas && metas.success) {
        ...
        // 根据表格配置文件，设置配置中的列的渲染， condition是渲染的条件判断
        ...
        // 获取表格行中删除，修改的操作ExecutorType
        const menus = (record, index) => {
          // 根据planState中的配置 去 获取接口中对象
    ...
    * getPlanMapData({ payload, callback }, { call, put, select }) {
      ...
      if (res && res.success) {
        ...
        opNotification.error('获取巡检人员失败，请重试或联系管理员！', false);
      }
    ...
    * setDataSource({ payload, callback }, { call, put, select }) {
      ...
      // 获取对应计划的数据展示结构
      let matchField = planCOnfig.matchField; // name
      // 按照对应的操作 去 匹配显示的字段，过滤掉columns中的字段
    ...
    * serverDispatch({ payload, callback }, { call, put, select }) {
      ...
      yield put({
        type: 'requestServer', payload: {
          requestParam: requestParam, operatePlan: operatePlan, url: 'dfs',
          serverMethod: operatorFunctionObj.serverMethocd, beforeServer: operatorFunctionObj.beforeServer
        },
        callback: operateObj[operatorFunctionObj.operatorCallbackName]
      });
    },
