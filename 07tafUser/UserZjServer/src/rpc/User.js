...
Test.UserImp = function () { 
    this._name   = undefined;
    this._worker = undefined;
}

Test.UserImp.prototype.initialize = function () { 
    //TODO:
}

Test.UserImp.prototype.onDispatch = function (current, FuncName, BinBuffer) { 
    switch (FuncName) {
        case 'login' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                var stReq = wup.readStruct("stReq", Test.LoginReq);
                var stRsp = wup.readStruct("stRsp", Test.LoginRsp, new Test.LoginRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Test.LoginReq);
                var stRsp = is.readStruct(2, false, Test.LoginRsp);
            }
            ...
            this.login(current, stReq, stRsp);
        ...
        case 'getUserList' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var stReq = wup.readStruct("stReq", Test.PageInfoReq);
                var stRsp = wup.readStruct("stRsp", Test.UserListRsp, new Test.UserListRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Test.PageInfoReq);
                var stRsp = is.readStruct(2, false, Test.UserListRsp);
            }
            current.sendResponse = function (_ret, stRsp) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                    var wup = new TafStream.UniAttribute();
            ...
            this.getUserList(current, stReq, stRsp);
        ...
        case 'getUserDetail' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var stReq = wup.readStruct("stReq", Test.QueryIdReq);
                var stRsp = wup.readStruct("stRsp", Test.UserInfoRsp, new Test.UserInfoRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Test.QueryIdReq);
                var stRsp = is.readStruct(2, false, Test.UserInfoRsp);
            }
            current.sendResponse = function (_ret, stRsp) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                    var wup = new TafStream.UniAttribute();
            ...
            this.getUserDetail(current, stReq, stRsp);
        ...
        case 'loginForAd' : {
            ...
            this.loginForAd(current, stReq, stRsp);

            return TafError.SUCCESS;
        }
        case 'getUserListForAd' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                var wup = new TafStream.UniAttribute();
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
            ...
            this.getUserListForAd(current, stReq, stRsp);
        ...
        case 'delUser' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.decode(BinBuffer);
                var stReq = wup.readStruct("stReq", Test.QueryIdReq);
                var stRsp = wup.readStruct("stRsp", Test.BasicRsp, new Test.BasicRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Test.QueryIdReq);
                var stRsp = is.readStruct(2, false, Test.BasicRsp);
            }
            current.sendResponse = function (_ret, stRsp) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                    var wup = new TafStream.UniAttribute();
            ...
            this.delUser(current, stReq, stRsp);
        ...
        case 'saveUser' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var stReq = wup.readStruct("stReq", Test.SaveUserInfoReq);
                var stRsp = wup.readStruct("stRsp", Test.SaveUserInfoRsp, new Test.SaveUserInfoRsp());
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var stReq = is.readStruct(1, true, Test.SaveUserInfoReq);
                var stRsp = is.readStruct(2, false, Test.SaveUserInfoRsp);
            }
            current.sendResponse = function (_ret, stRsp) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                    var wup = new TafStream.UniAttribute();
            ...
            this.saveUser(current, stReq, stRsp);

            return TafError.SUCCESS;
        }
        case 'getUserDetailForAd' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_VERSION) {
                var wup = new TafStream.UniAttribute();
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
            ...
            this.getUserDetailForAd(current, stReq, stRsp);
...
Test.UserImp.prototype.login = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('login function not implemented');
}

Test.UserImp.prototype.getUserList = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('getUserList function not implemented');
}

Test.UserImp.prototype.getUserDetail = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('getUserDetail function not implemented');
}

Test.UserImp.prototype.loginForAd = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('loginForAd function not implemented');
}

Test.UserImp.prototype.getUserListForAd = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('getUserListForAd function not implemented');
}

Test.UserImp.prototype.delUser = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('delUser function not implemented');
}

Test.UserImp.prototype.saveUser = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('saveUser function not implemented');
}

Test.UserImp.prototype.getUserDetailForAd = function (current, stReq, stRsp) {
    //TODO:
    assert.fail('getUserDetailForAd function not implemented');
}
